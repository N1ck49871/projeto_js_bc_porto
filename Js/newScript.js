...
function orderLeaderboard(playersList = []) {
    ...
    const playersMapped = [copyPlayers.shift()]
    for (let i = 0; i < copyPlayers.length; i++) {
        const player = copyPlayers[i]
...
function buildLeaderboard() {
    ...
    main.innerHTML = `<div class="all-leaderboards-container">
        ...
        <div class="medias-container">
            <span class="span-medias media-acerto" id="media-acerto">Média de acertos: <br></span><span class="span-medias media-erro" id="media-erro">Média de erros:<br> </span>
        </div>
    ...
    mediaAcerto.innerHTML += (pontuacaoTotal / players.length).toFixed(1)
    let mediaErro = document.querySelector("#media-erro")
    mediaErro.innerHTML += (10 - pontuacaoTotal / players.length).toFixed(1)
...
function evaluateQuiz() {
    ...
    const hoje = new Date()
    dados.data = `${hoje.getDate()}/${
        hoje.getMonth() + 1
    }/${hoje.getFullYear()} ${hoje.getHours()}:${hoje.getMinutes()}`
